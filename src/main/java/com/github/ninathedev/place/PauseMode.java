...
public class PauseMode implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, @NotNull Command command, @NotNull String s, String[] strings) {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            player.sendMessage("[place] "+Place.getInstance().getConfig().getString("messages.commands.pause-mode"));
        }
        Place.getInstance().getLogger().info("Pause mode enabled by: "+commandSender.getName());
        return true;
    }
}
